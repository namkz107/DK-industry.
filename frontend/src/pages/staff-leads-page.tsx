...
const nextLeadStatuses: Record<LeadStatus, LeadStatus[]> = {
  new: ["contacted", "lost", "spam"], qualified: ["contacted", "needs_analysis", "lost", "spam"], contacted: ["needs_analysis", "quoted", "lost", "spam"],
...
function LeadDetail({ lead, busy, error, onUpdate, onNote }: { lead: StaffLead; busy: boolean; error: Error | null; onUpdate: (body: Parameters<typeof staffClient.updateLead>[1]) => void; onNote: (content: string) => void }) {
  const [status, setStatus] = useState<LeadStatus | "">(""); const [message, setMessage] = useState(""); const [note, setNote] = useState("")
  useEffect(() => { setStatus(""); setMessage(""); setNote("") }, [lead._id, lead.status])
  const submitStatus = (event: FormEvent) => { event.preventDefault(); if (status) onUpdate({ status, message, lostReason: status === "lost" ? message : undefined }) }
  const nextOptions = nextLeadStatuses[lead.status].map(value => [value, statusNames[value]])
  return <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><div className="flex flex-col justify-between gap-4 md:flex-row"><div><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-black uppercase tracking-wider text-orange-700">{lead.code}</p><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-800">{statusNames[lead.status]}</span></div><h2 className="mt-2 font-display text-3xl font-bold text-emerald-950">{lead.name}</h2><p className="mt-2 text-slate-600">{lead.serviceType || "Chưa xác định dịch vụ"} · {lead.budget || "Chưa có ngân sách"}</p></div><Button onClick={() => onUpdate({ assignedTo: lead.assignedTo ? "unassigned" : "me" })} disabled={busy} variant={lead.assignedTo ? "outline" : "default"}><UserCheck className="size-5"/>{lead.assignedTo ? `Phụ trách: ${lead.assignedTo.name}` : "Nhận phụ trách"}</Button></div>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-2"><form onSubmit={submitStatus} className="space-y-4"><h3 className="text-lg font-bold text-emerald-950">Bước xử lý tiếp theo</h3>{nextOptions.length ? <><Filter value={status} onChange={value => setStatus(value as LeadStatus)} first="Chọn bước tiếp theo" options={nextOptions}/><select className="field-select" aria-label="Mức ưu tiên" value={lead.priority} onChange={event => onUpdate({ priority: event.target.value as Priority })}>{Object.entries(priorityNames).map(([value, label]) => <option key={value} value={value}>Ưu tiên: {label}</option>)}</select><label className="block text-sm font-bold text-slate-700"><span className="mb-2 flex items-center gap-2"><CalendarClock className="size-4"/>Lịch liên hệ tiếp theo</span><Input type="datetime-local" defaultValue={lead.nextFollowUpAt?.slice(0, 16) || ""} onBlur={event => onUpdate({ nextFollowUpAt: event.target.value })}/></label><Textarea value={message} onChange={event => setMessage(event.target.value)} placeholder={status === "lost" ? "Lý do không thành công (bắt buộc)" : "Ghi chú ngắn về bước xử lý"}/><Button disabled={busy || !status || (status === "lost" && message.trim().length < 3)}>Xác nhận bước này</Button></> : <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">Lead đã kết thúc, không còn bước xử lý tiếp theo.</p>}</form>
