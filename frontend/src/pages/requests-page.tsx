  ...
  return <CustomerShell title="Yêu cầu gia công & báo giá" description="Gửi thông số và bản vẽ một lần; mỗi yêu cầu có mã riêng để theo dõi xuyên suốt.">
    ...
    <div className="grid gap-6 xl:grid-cols-[.85fr_1.15fr]">
      ...
      <div><h2 className="mb-4 font-display text-2xl font-bold text-emerald-950">Lịch sử yêu cầu</h2>{requests.isLoading ? <Loading/> : !requests.data?.data.length ? <div className="rounded-3xl border border-dashed border-slate-300 bg-white py-14 text-center"><FileCog className="mx-auto size-12 text-slate-300"/><p className="mt-4 font-bold text-slate-700">Bạn chưa có yêu cầu nào.</p></div> : <div className="space-y-4">{requests.data.data.map(request => {
        ...
        return <article className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6" key={request._id}><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><div className="flex flex-wrap items-center gap-2"><span className="text-xs font-black uppercase tracking-wider text-orange-700">{request.code}</span><span className={`rounded-full px-3 py-1 text-xs font-black ${status.className}`}>{status.label}</span></div><h3 className="mt-3 font-display text-xl font-bold text-emerald-950">{request.title}</h3><p className="mt-1 text-sm text-slate-500">{typeNames[request.requestType]} · {dateTime(request.createdAt)}</p></div>{['submitted', 'need_more_info'].includes(request.status) && <Button size="icon" variant="ghost" title="Hủy yêu cầu" onClick={() => { if (window.confirm(`Hủy yêu cầu ${request.code}?`)) cancel.mutate(request._id) }}><XCircle className="size-5 text-red-600"/></Button>}</div><p className="mt-4 line-clamp-3 text-sm leading-6 text-slate-600">{request.description}</p>{request.attachments.length > 0 && <div className="mt-4 flex flex-wrap gap-2">{request.attachments.map(attachment => <button className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-slate-100 px-3 text-sm font-bold text-slate-700 hover:bg-orange-50 hover:text-orange-800" key={attachment._id} onClick={() => void download(request, attachment)}><Download className="size-4"/>{attachment.originalName}</button>)}</div>}<details className="group mt-4 border-t border-slate-100 pt-4"><summary className="flex min-h-10 cursor-pointer list-none items-center justify-between font-bold text-emerald-950">Tiến trình xử lý<ChevronDown className="size-5 transition group-open:rotate-180"/></summary><ol className="mt-3 space-y-3">{request.timeline.map((item, index) => <li className="flex gap-3 text-sm" key={`${item.at}-${index}`}><span className="mt-1 size-2 shrink-0 rounded-full bg-orange-500"/><div><strong>{statuses[item.status]?.label || item.status}</strong><p className="mt-1 text-slate-500">{item.message}</p><time className="mt-1 block text-xs text-slate-400">{dateTime(item.at)}</time></div></li>)}</ol></details></article>
