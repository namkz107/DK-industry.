...
const links = [{ to: "/", label: "Trang chủ" }, { to: "/dich-vu", label: "Dịch vụ" }, { to: "/du-an", label: "Dự án" }, { to: "/san-pham", label: "Sản phẩm" }]
...
export function SiteHeader({ onQuote }: { onQuote: () => void }) {
  ...
  return <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
    <div className="container-page flex h-[76px] items-center justify-between gap-4">
      <Link to="/" className="flex shrink-0 items-center gap-3" aria-label="DK Industry - Trang chủ"><span className="relative grid size-11 place-items-center rounded-xl bg-emerald-950 font-display text-base font-black text-white">DK<i className="absolute -bottom-1 -right-1 size-3 rounded-sm bg-orange-600" /></span><span className="font-display text-[15px] font-extrabold tracking-[.12em] text-emerald-950">DK INDUSTRY<small className="mt-1 block text-[9px] font-semibold tracking-[.16em] text-slate-500">ENGINEERING EXCELLENCE</small></span></Link>
